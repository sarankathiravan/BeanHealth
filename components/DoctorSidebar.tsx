...
const DoctorSidebar: React.FC<DoctorSidebarProps> = ({ activeView, setActiveView, isOpen, onClose }) => {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-white dark:bg-slate-800 shadow-md flex-shrink-0 flex flex-col transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="h-20 flex items-center justify-between px-4 border-b border-slate-200 dark:border-slate-700">
         <button onClick={() => setActiveView('dashboard')} className="flex items-center space-x-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-md -ml-1 p-1">
            ...
            <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Beanhealth</h1>
        </button>
        <button onClick={onClose} className="md:hidden p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700">
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-700">
        <p className="text-xs text-slate-400 dark:text-slate-500 text-center">© 2024 Beanhealth. All rights reserved.</p>
      </div>
    </aside>
  );
};
